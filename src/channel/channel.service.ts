...
export class ChannelsService {
  ...
  async createChannel(req: any): Promise<string | HttpException> {
    try {
      const GameInfo = await this.RedisClient.lrange(`${req.name}`, 0, -1);
      const password = await bcrypt.hash(req.password, 10);

      if ((await GameInfo).find((name) => name === req.name)) {
  ...
  async enterChannel(
    ...
    try {
      const GameInfo = await this.RedisClient.lrange(`${name}`, 0, -1);
      ...
      const checkIds = GameInfo.filter((value) => /^user id: \d+$/.test(value));
      ...
      for (let i = 0; i < (await checkIds).length; i++) {
        if ((await GameInfo)[i] === req.user_id) {
          throw new HttpException(
            {
              status: HttpStatus.BAD_REQUEST,
              error: "이미 존재하는 방입니다.",
